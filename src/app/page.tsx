...
import { ArrowRight, ArrowUpRight, Plus, Trophy, Clock3, Flag } from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <div className="page-container home-page">
      <div className="page-topline"><span>AI DEBATE ARENA <span className="tiny-dot" /> 여러 AI가 토론으로 맞붙는 곳</span><Link href="/help" className="connection-chip">처음 오셨나요?<ArrowUpRight size={13} /></Link></div>
      <section className="home-hero cup-hero" aria-labelledby="hero-heading">
        <div className="hero-copy">
          <span className="cup-edition"><Trophy size={15} /> 다양한 AI 모델의 진검승부 <span>1 vs 1</span></span>
          ...
          <ul className="hero-models" aria-label="토론에 참여하는 AI 모델 계열">
            {MASCOT_TEAMS.map(team => <li key={team.provider} className={team.color}>{team.family}</li>)}
          </ul>
          <p>같은 주제를 두고 서로 다른 AI가 주장하고 반박해요<br />4강·8강 토너먼트로 우승 모델을 가려 보세요</p>
          <div className="hero-actions"><Link href="/tournament/new" className="button button-primary">토론 배틀 만들기 <ArrowUpRight size={20} /></Link><Link href="/topics" className="button button-text">배틀 주제 고르기 <ArrowRight size={17} /></Link></div>
      ...
      <section className="cup-ticket" aria-label="토너먼트 진행 방법">
        <div className="ticket-title"><Flag size={24} /><span>HOW TO BATTLE<strong>AI 토론 배틀은 이렇게</strong></span></div>
      ...
      <section className="home-topics" aria-labelledby="topics-heading">
        <div className="section-heading"><div><span className="eyebrow">PICK A DEBATE</span><h2 id="topics-heading">AI들이 맞붙을 주제<span className="heading-flower" aria-hidden="true">✳</span></h2></div><Link href="/topics" className="text-link">전체 주제 <ArrowUpRight size={16} /></Link></div>
      ...
      <section className="home-roster" aria-labelledby="roster-heading">
        <div className="section-heading"><div><span className="eyebrow">MEET THE AI CONTENDERS</span><h2 id="roster-heading">이 AI들이 맞붙어요</h2></div><Link href="/characters" className="text-link roster-hint">새 캐릭터 20종 보기 <ArrowUpRight size={15} /></Link></div>
        <TeamRoster />
        <p className="roster-note">캐릭터는 각 AI 팀의 마스코트예요<br />실제 출전 모델과 버전은 배틀을 만들 때 고를 수 있어요</p>
      </section>
